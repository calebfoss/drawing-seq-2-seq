...
if len(glob(os.path.join(checkpoint_dir, 'ckpt_*'))) > 0:
	...
	en_in_x = layers.Input(shape=(None, 1,), dtype='int32', name='in_top_x')
	en_in_y = layers.Input(shape=(None, 1,), dtype='int32', name='in_top_y')
	en_in_p = layers.Input(shape=(None, 1,), dtype='int32', name='in_top_p')
	em_x = layers.Embedding(input_dim=512, output_dim=E_DIM, name='em_x')
	em_y = layers.Embedding(input_dim=512, output_dim=E_DIM, name='em_y')
	em_p = layers.Embedding(input_dim=5, output_dim=E_DIM, name='em_p')
	...
	en_flat_merged = layers.Reshape((-1, E_DIM * 3), name='en_flat_merged')(en_merged_layer)

	encoder = layers.LSTM(RNN_UNITS, return_state=True, name='encoder')
	encoder_outputs, state_h, state_c = encoder(en_flat_merged)
	encoder_states = [state_h, state_c]

	de_in_x = layers.Input(shape=(None, 1,), dtype='int32', name='in_bottom_x')
	de_in_y = layers.Input(shape=(None, 1,), dtype='int32', name='in_bottom_y')
	de_in_p = layers.Input(shape=(None, 1,), dtype='int32', name='in_bottom_p')
	...
	de_flat_merged = layers.Reshape((-1, E_DIM * 3), name='de_flat_merged')(de_merged_layer)

	decoder_lstm = layers.LSTM(RNN_UNITS, return_sequences=True, return_state=True, name='decoder')
...
in_top_x, in_top_y, in_top_p, in_bottom_x, in_bottom_y, in_bottom_p, out_x, out_y, out_p = \
	tf.placeholder(tf.int32, shape=[None, 1]), tf.placeholder(tf.int32, shape=[None, 1]), tf.placeholder(tf.int32, shape=[None, 1]), \
	tf.placeholder(tf.int32, shape=[None, 1]), tf.placeholder(tf.int32, shape=[None, 1]), tf.placeholder(tf.int32, shape=[None, 1]), \
	tf.placeholder(tf.int32, shape=[None, 1]), tf.placeholder(tf.int32, shape=[None, 1]), tf.placeholder(tf.int32, shape=[None, 1])
